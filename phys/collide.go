...
// MoveCircle moves a circle with a given velocity, handling collision
// with the sides of polygons.
func MoveCircle(c Circle, v Vector, sides []Side) Circle {
	for !v.Equals(Vector{}) {
		d, v2 := moveCircle1(c, v, sides)
...
// with any of a set of sides.  The return value is the distance that
// the circle moved, and the new velocity vector.
func moveCircle1(c Circle, v Vector, sides []Side) (float64, Vector) {
	hitPt := Point{}
	dist := math.Inf(1)

	for _, s := range sides {
		if d, pt, hit := CircleSideHit(c, v, s); hit && d < dist {
			dist = d
			hitPt = pt
		}
	}
...
// CircleSideHit returns information about the collision of a circle
// and the side of a polygon.  The return values are the distance
// along the velocity vector of the collision, the point on the polygon
// that collided, and a boolean that is true if there was a collision
// and false if not.
func CircleSideHit(c Circle, v Vector, s Side) (float64, Point, bool) {
